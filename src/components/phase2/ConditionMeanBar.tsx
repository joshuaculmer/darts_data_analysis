...
export function ConditionMeanBar({ stats }: Props) {
  ...
  return (
    <div className="chart-card">
      <h2>Mean Execution Skill per Condition (95% CI)</h2>
        ...
        <BarChart data={data} margin={{ top: 16, right: 24, left: 0, bottom: 8 }}>
          ...
          <Tooltip
            formatter={(value, _name, item) => {
              if (typeof value !== "number") return ["N/A", "Mean"];
              const count = (item.payload as typeof data[0] | undefined)?.count ?? 0;
              return [`${value.toFixed(2)} (n=${count})`, "Mean"];
            }}
      ...
      <p style={{ fontSize: 11, color: "#64748b", marginTop: -4 }}>
        Error bars show 95% confidence interval (1.96 × SE)
      </p>
    </div>
  );
}
